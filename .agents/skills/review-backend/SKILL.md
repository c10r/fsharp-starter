...
- Verify timestamp type policy is enforced.
- Ban `DateTimeOffset` in backend code (Domain/Application/Infrastructure/API/tests).
- Require UTC `DateTime` for persisted and DTO/domain timestamps in this starter.
- Require `rg -n "DateTimeOffset" src` to return no matches before merge.
